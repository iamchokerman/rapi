...
const encoded_embed_url = "aHR0cHM6Ly9yYWJiaXRzdHJlYW0ubmV0L3YyL2VtYmVkLTQv";
const encoded_referrer = "aHR0cHM6Ly9mbGl4aHEudG8v";
const encoded_image_url = "aHR0cHM6Ly9yYWJiaXRzdHJlYW0ubmV0L2ltYWdlcy9sb2FkaW5nLnBuZz92PTAuNg";
const encoded_getSourcesUrl = "aHR0cHM6Ly9yYWJiaXRzdHJlYW0ubmV0L2FqYXgvdjIvZW1iZWQtNC9nZXRTb3VyY2VzP2lkPQ==";
const embed_url = atob(encoded_embed_url);
const referrer = atob(encoded_referrer);
const image_url = atob(encoded_image_url);
...
const main = async (xrax: string) => {
  ...
  const canvas = {
    baseUrl: `${embed_url}mcAWNPptFcOb?z=`,
    width: 0,
    height: 0,
    style: {
      style: {
  ...
  const nodeList = {
    image: {
      src: `${embed_url}images/image.png?v=0.1.4`,
      height: 50,
      width: 65,
      complete: true,
    },
  ...
  let script_url = `${embed_url}z1AOmWCJVgcy?z=`;
  ...
  const V = async () => {
    let Q0 = await wasmLoader(image_url);
    try {
      wasmLoader.greet();
    } catch (error) {
      console.log(error);
  ...
  const newReq = async (xrax: string) => {
    ...
    let keys = await V();
    let getSourcesUrl = atob(encoded_getSourcesUrl) + xrax + "&v=" + fake_window.localStorage.kversion + "&h=" + fake_window.localStorage.kid + "&b=1676800512";
